...
class CanteenRepository {
    suspend fun getCanteens(token: String): List<Canteen> {
        return try {
            val response =
                RetrofitClient.canteenApi.getCanteens(token = "Bearer $token")
    ...
    suspend fun getCanteensById(id: Int): Canteen? {
        return try {
            val response = RetrofitClient.canteenApi.getCanteenById(id)
            if (response.isSuccessful) {
                response.body()
            } else {
    ...
    suspend fun getAllMenuCategoryByCanteen(id: Int): List<MenuCategory> {
        return try {
            val response =
                RetrofitClient.canteenApi.getAllMenuCategoryByCanteen(id = id)
            if (response.isSuccessful) {
                response.body() ?: emptyList()
            } else {
                emptyList() // bisa juga lempar exception
